...
from pysamosa.utils.data_calculations import india
...
def set_phase(start, end, phase_num):
    ...
    return pd.DataFrame(arr_phase_num,
                        index=index,
                        columns=['collocation_phase'])
...
def fixed_raster_merge(ds_points, ds_raster, keys='site', xy=None):
    ...
    for key in ds_points[keys]:
        ...
        try:
            cell = ds_raster.sel(indexers={'latitude': latitude, 'longitude': longitude}, method='nearest')
            cell = cell.drop_vars(names=['longitude', 'latitude'])
            cell.coords[keys] = key
        except KeyError:
            cell = ds_raster.sel(indexers={'x': latitude, 'y': longitude}, method='nearest')
...
def find_nearest_point(df_coords_1, df_coords_2, tolerance=0.005):
    ...
    tree = cKDTree(list(zip(df_coords_1['latitude'],
                            df_coords_1['longitude'])))

    for i, row in df_coords_2.iterrows():
        dist, idx = tree.query([row['latitude'], row['longitude']], k=1, distance_upper_bound=tolerance)
...
def fixed_point_merge(ds_points, ds_merge, tolerance=0.005, keys='site', xy=None):
    ...
    if xy is not None:
        ...
        df_coords_points = ds_points[['latitude', 'longitude']].to_dataframe().dropna().drop_duplicates()

    df_coords_merge = ds_merge[['latitude', 'longitude']].to_dataframe().dropna(subset=['latitude',
                                                                                        'longitude']
                                                                                ).drop_duplicates()
    df_matched = find_nearest_point(df_coords_merge, df_coords_points, tolerance=tolerance)
...
def open_and_merge(in_path, file_list):
    ...
    :rtype: xarray.Dataset
    """
    return xr.merge([xr.open_dataset(os.path.join(in_path, f'{file}.nc')) for file in file_list])
...
def open_and_merge_raster(in_path, ds_ref, raster_list, xy=None, keys='site'):
    ...
    """
    return xr.merge(
        [fixed_raster_merge(ds_ref, xr.open_dataset(os.path.join(in_path, f'{raster}.nc')), xy=xy, keys=keys)
...
def make_history(ds):
    ...
    :rtype: xarray.Dataset
    """
    ds_history = ds.drop(['land_uses', 'settlement', 'district', 'site', 'state', 'cluster',
                          'settlement_type', 'is_collocation_site', 'latitude', 'longitude'])
    ds_history = ds_history.to_dataframe().reset_index().drop(['position'], axis=1).set_index(
        ['sensor', 'time']).drop_duplicates().dropna(how='all').to_xarray()
    ds_history_season = india.get_season(
        ds_history['time'].to_dataframe().reset_index(drop=True).set_index('time'), False).to_xarray()
...
def make_phases(ds, dict_phases):
    ...
    :rtype: xarray.Dataset
    """
    df_collocation_phase = pd.concat([set_phase(start, end, i + 1) for start, end, i in
                                      zip(dict_phases['start'], dict_phases['end'], range(len(dict_phases['start'])))])

    ds_phases = ds.where(ds.site == 'IITD').dropna(dim='position', how='all').dropna(dim='time', how='all')
    ds_phases = ds_phases.drop(['land_uses', 'settlement', 'district', 'state', 'cluster',
                                'is_collocation_site', 'latitude', 'longitude',
                                'settlement_type', 'site'])
    ds_phases = ds_phases.to_dataframe().reset_index().drop(['position'], axis=1).set_index(
        ['sensor', 'time']).to_xarray()
    ds_phases_season = india.get_season(
        ds_phases['time'].to_dataframe().reset_index(drop=True).set_index('time'), False).to_xarray()
    ds_phases = xr.merge([ds_phases, df_collocation_phase.to_xarray(), ds_phases_season]).set_coords(['collocation_phase',
                                                                                                      'season'])
    return ds_phases
...
def make_collocation(ds):
    ...
    ds_collocation = ds_collocation.drop(
        ['settlement', 'district', 'state', 'cluster', 'is_collocation_site', 'land_uses', 'settlement_type'])

    df_pa = pd.concat([
        ds_collocation[['a']].to_dataframe().reset_index().drop(['position', 'sensor',
                                                                 'latitude', 'longitude'],
                                                                axis=1).rename(columns={'a': 'pa'}),
        ds_collocation[['b']].to_dataframe().reset_index().drop(['position', 'sensor',
                                                                 'latitude', 'longitude'],
                                                                axis=1).rename(columns={'b': 'pa'}),
    ])

    df_pa = df_pa.groupby(['site', 'time']).agg(pa_raw=('pa', 'mean'),
                                                cv=('pa', lambda x: x.std() / x.mean()))

    df_rh = ds_collocation[['rh']].to_dataframe().reset_index().drop(['position', 'sensor',
                                                                      'latitude', 'longitude'],
                                                                     axis=1).groupby(['site', 'time']).mean()

    df_disagreement = ds_collocation[['disagreement']].to_dataframe().reset_index().drop(['position', 'sensor',
                                                                      'latitude', 'longitude'],
                                                                     axis=1).groupby(['site', 'time']).mean()

    df_collocation = pd.concat([df_pa, df_rh, df_disagreement], axis=1).dropna(how='all')

    ds_collocation_meta = ds_collocation[['latitude', 'longitude']].drop('sensor').to_dataframe().reset_index(
        drop=True).set_index('site').drop_duplicates().to_xarray()
    ds_collocation_season = india.get_season(
        ds_collocation['time'].to_dataframe().reset_index(drop=True).set_index('time'), False).to_xarray()

    ds_collocation = xr.merge([df_collocation.to_xarray(),
                               ds_collocation_meta,
                               ds_collocation_season]).set_coords(['latitude', 'longitude', 'season'])
...
def make_deployment(ds):
    ...
    :rtype: xarray.Dataset
    """
    ds_deployment = ds.where(ds.is_collocation_site == 0).dropna(dim='position', how='all').dropna(dim='time',
                                                                                                   how='all')
    df_deploy = ds_deployment.to_dataframe().reset_index().drop(['position'], axis=1)
    df_deploy.loc[:, 'pa_raw'] = (df_deploy['a'] + df_deploy['b']) * 0.5
    ds_deploy = df_deploy.groupby(['site', 'time']).agg({'pa_raw':'first', 'a': 'first', 'b': 'first', 'rh': 'first',
                                                         'disagreement':'first',
                                                         'a_flag': 'first', 'b_flag': 'first', 'rh_flag': 'first',
                                                         'sensor': 'first'}).to_xarray()
    ds_deploy_meta = ds_deployment[['site']].to_dataframe().reset_index(drop=True).set_index('site').sort_index().drop(
        ['sensor'], axis=1).drop_duplicates().to_xarray()
    ds_deployment = xr.merge([ds_deploy, ds_deploy_meta])

    ds_deployment = ds_deployment.set_coords(
        ['land_uses', 'settlement', 'district', 'state', 'settlement_type', 'cluster', 'latitude', 'longitude']).drop(
        ['is_collocation_site'])

    ds_deployment_season = india.get_season(
        ds_deployment['time'].to_dataframe().reset_index(drop=True).set_index('time'), False).to_xarray()
...
def merge_reference(in_path):
    ...
    ds_reference_rasters = open_and_merge_raster(in_path, ds_reference,
                                                 ['era', 'tropomi', 'martin', 'ghsl'],
                                                 keys='site')
...
def merge_phases(in_path, dict_phases):
    ...
    ds_bam = fixed_point_merge(ds_phases, xr.open_dataset(os.path.join(in_path, 'bam.nc')),
                               xy=[28.5468, 77.1906])
...
def merge_collocation(in_path):
    ...
    ds_collocation = xr.merge([ds_collocation, xr.open_dataset(os.path.join(in_path, 'legacy.nc'))])
    ds_rasters = open_and_merge_raster(in_path, ds_collocation,
                                       ['era', 'tropomi', 'martin', 'ghsl'],
                                       keys='site')
...
def merge_deployment(in_path):
    ...
    ds_raster = open_and_merge_raster(in_path, ds_deployment,
                                      ['era', 'tropomi', 'martin', 'ghsl'],
                                      keys='site')
