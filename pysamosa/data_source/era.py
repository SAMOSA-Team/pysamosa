...
def calculate_rh(ds):
    ...
    return 100 * (np.exp((17.625 * d2m) / (243.04 + d2m)) / np.exp((17.625 * t2m) / (243.04 + t2m)))
...
def format_era(in_path, crs='EPSG:4326'):
    ...
    ds_era = xr.open_mfdataset(glob.glob(os.path.join(in_path, '*_ERA5.nc')),
    ...
    ds_era['time'] = ds_era['time'].to_index().tz_localize('UTC').tz_convert('Asia/Kolkata').tz_localize(None)
    ...
    spd10 = xr.Dataset({
        "spd10": (("time", "latitude", "longitude"), calculate_spd(ds_era.u10.values, ds_era.v10.values))
    },
        coords={
            "time": ds_era.time,
            "latitude": ds_era.latitude,
    ...
    wd10 = xr.Dataset({
        "wd10": (("time", "latitude", "longitude"), calculate_dir(ds_era.u10.values, ds_era.v10.values))
    },
        coords={
            "time": ds_era.time,
            "latitude": ds_era.latitude,
